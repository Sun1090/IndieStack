...
| `pnpm check:fields` | Enforce shared `FormField` / `NativeSelect` usage, ban raw selects, copied control classes and direct label imports, and require `method="post"` on JS-driven `<form onSubmit>` |
