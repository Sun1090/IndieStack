...
| `appark` | Appark APM | `NEXT_PUBLIC_APPARK_API_KEY`、`NEXT_PUBLIC_APPARK_ENDPOINT`、`NEXT_PUBLIC_APPARK_SAMPLE_RATE` | 两项凭据必须同时存在才能启用 APM；可选采样率必须在 0 到 1 之间，缺省为 1。凭据只配置一项或采样率非法会标记 `misconfigured`；凭据不完整时运行时旁路，采样率非法时回退到 1 |
...
| `appark: misconfigured` | 补齐缺失的 Appark key，或删除两项凭据让 APM 保持关闭。若是采样率非法，把 `NEXT_PUBLIC_APPARK_SAMPLE_RATE` 设为 0 到 1 之间的数字（默认 1）。 |
