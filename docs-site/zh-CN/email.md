...
「跳过」同时也是这一行离开队列的方式（A05）：同一个分支会为本轮放弃的那批 id 写入
`notifications.email_skipped_reason`，它们因此不再占住按 `created_at` 升序的拉取队首。
原因既不是投递尝试也不是发送：这些行永远不会有 `email_sent`，重试计数也停在原处——
「这个用户没有邮箱」不是一次重试能修好的故障。代价写进库注释也写在这里，而不只留在代码注释里：
该用户后来补了邮箱、或把类型重新打开，**已经跳过的那批通知不会被复活**——它们留在站内，
下一条通知照常寄出。原因取值集合有一份在 `src/lib/notifications/types#EMAIL_SKIP_REASONS`、
有一份在 `supabase/migrations/034_email_skip_reason.sql` 的 `CHECK` 里，有一条测试直接读迁移文件对账，
所以漂移的表现是生产写入被拒绝、或者这里红一条，而不是那一列悄悄变宽。
...
与 Web Push 不同，这里没有行龄上界，因为「N 天后丢掉队列里的邮件」改变的是送达语义——A05 这次定的
是「判定寄不出去就当场出队」，没有顺手给待发邮件加丢弃上限，那一条仍然是敞开的。第三种
`stage="skip"` 说的是出队那笔写入本身失败：本轮照旧按「跳过」计数、整轮照常收尾，但那些行还留在队列
里，下一轮会再被拉起来一次。三种情形都不再允许抹掉本轮自己的记录：中途失败的运行会照实记下当时的 `pulled` / `sent` /
...
过滤，所以面板上报的年龄说的就是那支队伍。这一条说的是可见性。除了上面那几个数，面板还拆开两笔「已经离开队列」的账：worker 当场判定寄不出去、按
`email_skipped_reason` 分组的那批，以及在站内先被读掉的那批（队列条件含 `is_read = false`，所以它们
连 `email.backlog` 都不再计入——只看积压数会把一次堵塞读成一次缩小）。这两个数都不改变发什么，
它们说的是哪些行走了、以及为什么走。
