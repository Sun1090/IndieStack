...
- **预热的清单不再手抄**：唯一的清单是 `src/lib/testing/e2e-warm-routes.ts` 里的 `WARM_ROUTES`，
  `e2e/support/warm-up.ts` 只负责「逐台、一台内逐条 GET」。`e2e-warm-routes.test.ts` 从 `e2e/*.spec.ts`
  静态读出**实际被导航到**的路由（`${appUrl()}<path>` 的尾巴；`/api/*`、含 `${}` 的动态段、404 探针与
  `waitForURL` 的 glob 都不算），与清单双向对账：漏预热报 `E2E_WARM_MISSING`，清单里已经没人导航的报
  `E2E_WARM_STALE`，读不到 spec / 一条都没解析到各自失败封闭。第一次对账量出的差距是 **4 条预热 vs 16 条被导航**
  ——那 12 条页面的冷编译一直记在「恰好第一个打到它的那条用例」头上。
  两个支撑口径也是量出来的：mock 模式下未登录 GET `/dashboard/**` 返回 **200**（不是 307 回登录页），
  所以预热真的编译到那个页面本身；一条没预热的路由首次命中约 1.2s（`/pricing`，`next.js: 1215ms`）。
  **但本机没能为这次改动作证**：清空 `.next-e2e-*` 的三次冷启动里，旧清单红 2 条、新清单分别红 2 条和 4 条，
  每次红的用例都不一样（`responsive.spec.ts` 的移动端抽屉正是 #117 在修的位置）——本机并行冷启动的噪声
  比这次改动的效果大，结论要等 CI 的每周并行基线。也别顺手把几台服务器的预热并发化：那次对照把
  「清单从 4 条变 16 条」和「3 台并发预热」一起改了，结果多出 3 条登录导航超时，两个变量没分离开。
