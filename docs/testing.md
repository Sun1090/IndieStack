...
- 同一个调用图还顺带现量限频：`node scripts/check-route-auth.js --rate-limit-report` 逐条打印
  `id [家族] [限流器绑定]` 并给分母。它**不做判定**——哪些写入端点必须有窗口是产品判断
  （roadmap C12 的前置 ①），唯一的失败封闭是「一条都没匹配到」，因为那更可能意味着判据自己坏了。
