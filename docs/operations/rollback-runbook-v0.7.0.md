...
- **无新增迁移**：v0.7.0 未引入数据库变更，回滚只需回退应用部署；数据库保持向前兼容。
- **Web Push 回退开关**：清除 `NEXT_PUBLIC_VAPID_PUBLIC_KEY` 或 `VAPID_PRIVATE_KEY` 即可关闭推送
  通道，站内通知与邮件不受影响；已被浏览器创建的订阅在下次投递时返回 `not-configured` 并保持未投递，
  不会产生死信堆积。
- **订阅数据不随回滚删除**：`public.push_subscriptions` 中的数据保留，重新配置凭证后即可恢复投递；
