...
| 队列可观测性            | 新队列无积压/死信/失效端点指标                                     | `push.backlog`（阈值 500）、`push.delivery.dead`、`push.endpoint.revoked`、`cron.push-retry.*` | `sentry-alerts.md` 已记录契约  | 已补齐               |
...
4. 发布前外部条件不变：VAPID 密钥对、HTTPS 站点、`CRON_SECRET`。
...
2. 在具备 VAPID 密钥与 HTTPS 的环境完成一次真实浏览器推送链路（订阅 → 投递 → 瞬时失败退避重试 →
   关闭订阅/失效端点 → 死信），并记录结果。
3. 以真实 `CRON_SECRET` 触发一次 `/api/cron/push-retry`，确认返回脱敏计数且队列下降。
