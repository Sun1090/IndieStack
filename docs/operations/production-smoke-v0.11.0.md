...
- 目标 commit：取证据的当时**无法从响应确定**——生产跑的是 `/api/health` 尚未暴露 `commit` 字段的构建，
  ...
  **2026-09-25 起这一格有三种读法**（以前全印成 `unknown`，得分清是查部署记录才能做到的事）：
  `not-reported` = 那个构建根本没有这个字段，即**部署滞后**（正是上面那段推断现在由脚本自己说）；
  `no-build-env` = 字段在但值为空，即构建时没拿到 git 变量（平台侧配置，与新旧无关）；
  剩下才是短 SHA。证据文件里另存了一格 `commitReported`（键在不在），
  读旧产物时**缺这一格要按 `not-reported` 处理**，不能读成 `no-build-env`。
