...
- **Vercel Hobby 构建配额（截至 2026-09-22T10:50Z 仍在限流中）**：`indie-stack` 项目已被
  `Deployment rate limited — retry in 24 hours` 挡住**两次**——09-21T18:03Z 起一次，09-22T09:31Z
  （`main` 推进到 `8037bbd` 之后）又一次，10:50:02Z 再推 `main` 时同一条描述仍在；同一分钟的
  `indie-stack-docs-site` 却返回 `Deployment has completed`，说明**限流按项目计**。
  限流期间 `main` 的推送不产生生产部署，被限流的构建也不会排队：窗口结束后必须再触发一次部署才有证据。
