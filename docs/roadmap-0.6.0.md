...
> **进度（2026-09-22）· D 域实况核对**（逐项看代码，不看注解）：D04 是真实门禁（`check:i18n` 扫描 854 个静态翻译调用，动态 `t(\`...\`)` 按设计跳过——**这条盲区已于 2026-09-22 由 `pnpm check:dynamic-keys` 补上**，9 条契约以代码常量为权威取值集合校验双语键存在、无孤儿键、且源码中每个动态模板都已登记）；D05 已成立（`app-locale` cookie 由 `locale-switcher.tsx` 写入、`src/i18n/request.ts` 读取、`<html lang>` 跟随，`localePrefix: "never"` 所以不存在 URL 前缀分支）；D07 已成立（zod 消息都是 `actions` 键）；D09 = G07 的 `e2e/keyboard.spec.ts`；D10 = `check:a11y` 静态审计 + `e2e/a11y.spec.ts` 运行时 axe。D06 已完成（`e2e/smoke.spec.ts` 新增「切换到简体中文后页面真的渲染中文文案」一条：cookie=`zh-CN` + `<html lang>` + 首屏中文 Badge + 英文原文消失；此前只有「切到默认 en + 断言 cookie」两条，证明不了消息加载）。D08 见下方进度条目（D 域已完成）。
...
> **进度（2026-09-22）· D10 复核：静态 a11y 门禁此前形同虚设，已修复**：对 `check:a11y` 做可达性复核时发现
> 它的图标按钮规则**结构上不可能命中**——外层 `if` 要求 children 里不存在任何 2+ 字母的连续串，
> 内层判定又要求组件名（`MoreHorizontal`、`<svg`）存在，两者互斥；而它和同类门禁不同，**不打印任何计数器**，
> 所以每轮「✅ 无未标注的图标按钮」在 CI 日志里看不出它其实什么都没做。真实后果是 3 个 `size="icon"`
> 且没有任何可访问名称的按钮长期在线（admin 用户表的改角色下拉触发器、新建项目与新建团队的返回按钮），
> 屏幕阅读器只会念「按钮」。规则本体重写为 `src/lib/ui/a11y-rules.ts`（纯函数）+ `scripts/lib/a11y-check.js`（IO），
> 判定保守优先（去掉自闭合图标、剥掉 `Link`/`span` 等透传容器后什么都不剩才算纯图标按钮，
> 因此 `{t("apiKeys.create")}` 算有文本不报），扫不到文件按 `A11Y_NO_FILES` 失败封闭，
> 输出带 `scannedFiles / buttons / iconOnlyButtons` 计数器；4 项变异测试（规则永假、去掉失败封闭、
> 可访问名称恒真、img 规则失效）全部让 28 条单测变红，证明它真的会失败。
...
> 要求应用层保持 0 处、扫不到文件即失败封闭。评估写入 `docs/architecture/10-i18n.md`
> 「书写方向与长文本」：`src/components/ui/**`（35 处 / 9 文件，shadcn 基元随上游升级覆盖）
> 与 `translate-x-*` / `origin-left`（**Tailwind v4 无逻辑等价物**）作为已知残留显式登记，不假装解决；
> 长文本由 `e2e/responsive.spec.ts` 的 375/768/1280 无横向溢出断言守住。

> **进度（2026-09-22）· D01 已完成**：`pnpm check:glossary` + `docs/architecture/10-i18n.md` 的「术语表」小节
> （14 个术语，文档与 `GLOSSARY` 常量双向逐字相等）。判定按「英文侧命中术语 → 中文侧不得用禁止变体」，
> 且只在指定译法缺席时算违规；全库逐条核对后真正的漂移只有 2 处（「账号」→「账户」、「拥有者」→「所有者」），已修。
...
47. E07 告警阈值与去重（完成：新增 `src/lib/observability/ops-metrics.ts` 固化 `ops.supabase.restore` 的指标名与 `noop`/`restore`/`wait`/`escalate`/`skipped` 动作取值，`runRestoreCycle` 的所有终态统一经 `complete()` 出口上报 `value=1` 计数样本，修掉「配置缺失/状态查询失败零样本」与「escalate/skipped 恒为 0、计数型告警永不触发」两条盲区；新增 `alert-thresholds.test.ts` 把运维文档登记的 `email.backlog` / `push.backlog` 500 阈值与代码常量钉在一起，并校验恢复指标的单位、维度与三条按 `action` 分流的告警登记；运维文档补齐告警规则与按动作去重说明）
...
77. H07 审计日志索引复审（完成：`docs/db/index-review.md` 以 20 万行真实数据 EXPLAIN 定案——`idx_audit_logs_created_at` 覆盖仓库层唯一查询形态（`select * + order by created_at desc + range`）为 0.082ms / 53 buffers，并记录 `(user_id, created_at desc)` 的补索引触发条件；`audit-logs.ts` 默认关闭 `withExactTotal`，因为 `count(*)` 是这套查询里唯一随表无限增长的开销（Parallel Seq Scan 12.99ms / 6956 buffers））
78. H08 数据保留与删除策略（完成：迁移 `032_data_retention_erasure.sql` 新增 `erase_user_data(uuid)`——删除 `api_usage`（含 `ip_address`）、按邮箱删除 `contact_messages`、匿名化 `audit_logs`（置空 `user_id` 与指向本人的 `entity_id`、`metadata` 剔除 PII 键但保留行为事实），补齐 `cleanup_old_api_usage()` 90 天 / `prune_deleted_upload_objects()` 30 天 / `cleanup_resolved_contact_messages()` 365 天三条保留期与两个局部索引，四个函数建函数即收回客户端 `EXECUTE`；应用侧 `src/lib/account/deletion.ts` 固化「先擦除再删号」的失败语义（擦除失败中止、事后审计补记失败不回滚成功），`DELETE /api/user` 与 `deleteAccountAction` 共用编排，设置页补上此前只有文案没有实现的「危险区域」；`src/lib/privacy/data-policy.ts` 为保留天数/cron 任务名/PII 键/确认短语的单一事实来源，40 条契约测试把它与迁移 SQL 和 `docs/db/retention.md` 双向钉死；顺带查实并记录 pg_cron 在本地与云端项目均未安装，此前所有 SQL 侧保留期从未真正执行）
...
98. J08 发布后回滚演练
99. J09 v0.6.0 退出报告（完成：`docs/operations/release-exit-report-v0.6.0.md`——100 项任务与 6 条退出标准逐条对回代码、门禁与执行记录，结论 91 达成 / 7 部分达成 / 2 未达成（E03 已于 2026-09-22 随 digest 定案关闭，报告现为 92 / 6 / 2），并核对出「digest 错峰门控与每天一次的调度不兼容导致摘要邮件对除 UTC-1 外所有用户不投递」这一 P0；v0.6.0 当年是先发版、后补报告）
