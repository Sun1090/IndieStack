...
## 2026-09-23 — C08-c 重量：邻居缺陷现在有数了，门禁还没有

- 里程碑 / 版本：v0.12.0 / C08-c（#42 的前半：判据与测量，不含接线）。
- 分支 / commit：`measure/c08c-unbound-reads`（栈在 #103 之上）。
- 状态：DONE（PR 待 review 合并）。
- 为什么先重量而不是先接门禁：roadmap 里那条「接线时实测到 12 处」是 C08 接线期的产物，
  中间已被三批清偿改过（invitations 两处、checkout 两处随别的 PR 消失）。
  照一份过期清单开工 = 白跑两个文件 + 漏掉真实规模。这一族的判据也和 C08 不同：
  C08 判的是**断言**（类型上宣称不会有 error），这里判的是**解构压根没取 `error`**——
  没有断言可看，所以 C08 门禁对着它一直是绿的。
- 量出来的数（`--unbound`）：358 个文件 / **160 处** awaited 查询结果的解构绑定 /
  其中 **23 处不绑 `error`** / **0** 个文件因语法诊断被跳过。
  抽样逐行看了 5 处（`actions/team.ts:36`、`dashboard/page.tsx:44`、`admin/page.tsx:47`、
  `permission-gate.tsx:86`、`webhooks/stripe/route.ts:256`）：**误报 0**。
  失败方向全一样——读失败长成「没有这个团队」「0 个用户」「这个人不是管理员」「这张发票没有归属」。
  `permission-gate.tsx` 那 2 处与 C08 的 `justified` 同源（客户端组件无法 5xx），门禁必须留同样豁免。
- 一个副产品，登记为 #49：`keepsErrorChannel()` 只看断言类型里有没有 `error:` 成员，
  所以 `as { data: X; error: null }`（明写「error 恒为 null」）被当合规放行——与「抹掉 `error`」
  是同一句谎，只是换了拼写。**这条先记下我自己在数它时犯的错**：第一次用
  `grep "error: null;"` 数出「恰好 5 处」，写进文档后拿另一种拼写重数才发现——分号把单行写法
  `error: null };` 排除在外，`actions/team.ts` 实际出现 7 次而它在 23 处清单里只占 6 处。
  一个数错的规模比没有规模更危险，因为它会被人拿去排优先级。#49 里已改成「必须按 AST 重量」。
- 刻意没做的那一档：「绑了 `error` 却从不使用」。判它需要作用域分析；用全文标识符计数近似会把
  `catch (error)` 一起数进去，得到一个**只会漏报**的假指标。写出来又只用来给人当依据，
  比不写更糟，所以报告里没有它，只写明它没测。
- 验证：`pnpm -s type-check` → exit 0；`npx vitest run src/lib/security/query-error-channel.test.ts`
  → 18 passed（新增 8 条，其中一条打在真实仓库上：两侧都必须量到、跳过数必须为 0，
  这样规则被调空时它会红）。变异核对 4 项（V1–V4）逐项红：判断恒真、只认本地变量名不认属性名
  （`{ error: boom }` 这种改名绑定会漏）、语法诊断文件不计数、去掉 `await` 要求
  （未 await 的 builder 断言被误伤）。
  `pnpm lint` / `pnpm type-check` → exit 0；`CI=true pnpm check:all` → **exit 0**（38 道门禁、
  208 个测试文件全过）；`pnpm build` → exit 0。这条不接任何门禁、不改运行期行为，
  所以门禁那侧的唯一变化是 `check:query-errors` 仍然绿（新规则是加法，不在它的判定路径上）。
- 下一步：按影响面清这 23 处（每批自带用例与变异核对），清到只剩 `justified` 时再接门禁；
  #49（`error: null` 也算抹除，先按 AST 重量）与 #44 的 checkout 半边各自等依赖。
  **这一批最值得留下的不是那 23 个数，而是那个被自己推翻的「5」**：它写进文档之后我换一种拼写
  重数才发现漏了单行写法——一个数错的规模比没有规模更危险，因为下一个动手的人会照它排优先级。
  这条与 memory 里「declare green 之前先重量一遍」是同一件事，但这次是它抓到了我自己的第一次测量。
- 更新时间：2026-09-23（UTC）。

## 2026-09-23 — C08-c 第一批清偿：项目页 4 处读取，顺带量出计数器自己的盲区

- 里程碑 / 版本：v0.12.0 / C08-c 清偿（#42 的后半：一批一批清，门禁排在清完之后）。
- 分支 / commit：`fix/c08c-project-reads`（栈在 #104 之上）。
- 状态：DONE（PR 待 review 合并）。
- 清掉的是哪 4 处：`dashboard/projects/[id]/page.tsx` 两处（`team_members`、`projects`，原先都写着
  `as unknown as { data: …; error: null }`）、`dashboard/projects/page.tsx` 两处（归属一处同样明写
  `error: null`，项目列表一处**连断言都没有**）。失败方向是同一个：一次抖动长成 404「这个项目不存在」
  或「你还没有项目」，而 404 与空列表都是**终态**——用户不会去重试，会以为数据没了。
  列表那一路更糟：归属读成 null 时三元表达式干脆**跳过**第二次查询，故障与「真没有团队」给出同一个页面。
- 改法：绑定 `error`，故障在渲染之前 `throw`（页面交给 `dashboard/error.tsx` 的重试入口），
  `notFound()` / 空列表只留给真缺行。两条合法终态各自有用例钉住——只测「读失败要报错」不够，
  那样「一律抛错」也能全绿。
- 顺手抓出的一件事：`--unbound` 从 23 处降到 20 处，**而这批实际硬化了 4 处**。差的那一处是
  `const { data } = cond ? await query : { data: [] }`——`unwrapAwait` 拿到的是条件表达式，
  于是它连 160 的总数都没进过。这份清单从此要按**下界**读。已补一条特征化用例把盲区钉在测试里
  （判不到 → 断言 `total === 0`；将来修好计数器，这条用例逼着一起改），报告页脚与
  roadmap C08-c、testing.md 同步改成四条盲区。
- 下一步顺序因此插了一步：**先修计数器**（走进条件表达式的两个分支），用真实规模重排剩余批次，
  最后才接门禁。拿一份知道自己偏小的清单去排优先级，正是 #49 那条「数错的规模比没有规模更危险」的
  同一个错，只是这次偏的是 AST 计数器而不是 grep。
- 验证：`pnpm -s lint` / `pnpm -s type-check` → exit 0；`CI=true pnpm check:all` → **exit 0**
  （38 道门禁 / 210 个测试文件 / 2385 条用例）；`pnpm build` → exit 0。
  新增用例 11 条：两个页面各 5 条（`projects/page.test.tsx`、`projects/[id]/page.test.tsx`）+ 规则 1 条。
  变异核对 6 项（删三条守卫、把 `notFound()` 改成无条件、把合法空态改成故障）**全部被杀死**且红在
  对应用例上。第一版变异脚本按 `×` 抓失败用例名，而 dot reporter 打的是 `FAIL … > 用例名`，
  于是 6 条一律误报成「红了但不是这条断言」——脚本自己也要被变异，不然它给出的"全被抓到"是假的。
- 更新时间：2026-09-23（UTC）。

## 2026-09-23 — C08-c 计数器补漏：那两份「23 处」都是下界

- 里程碑 / 版本：v0.12.0 / C08-c（#50，排在清偿批次之前，因为规模本身是假的）。
- 分支 / commit：`fix/c08c-counter-conditional`（栈在 #105 之上）。
- 状态：DONE（PR 待 review 合并）。
- 为什么先修它：上一批清了 4 处读取，清单却只降 3 处。原因是判据只认「一条解构对着一条 awaited 链」，
  三种写法整体看不见——条件表达式包住的链、`await Promise.all([chain, …])` 的数组解构、
  数组元素上再盖 `as unknown as { data }` 的链。**拿着知道自己偏小的清单排优先级，和 #49 那条
  「数错的规模比没有规模更危险」是同一个错**，只是这次偏在 AST 计数器上而不是 grep。
- 修完重量的真数：358 文件 / **165 处**解构 / **24 处不绑 `error`** / 0 个文件跳过（原先 160 / 20）。
  新量出的 4 处**逐行看过、误报 0**：`dashboard/page.tsx` 的 api_usage 计数、user_sessions 计数、
  最近 5 条通知（这条还额外盖着一层抹掉 `error` 的断言，见下面的门禁盲区），
  `settings/page.tsx` 的设备列表——失败方向仍是「读失败长成 0 次调用 / 0 台设备 / 没有通知」。
- 两个实现细节是被抓出来的，不是设计出来的：`Promise.all` 的参数是**一个数组字面量**，第一版按
  `node.arguments` 配对 → 一条链都认不出（探针脚本抓到）；数组字面量里的空缺位不能滤掉，
  滤掉之后下标与解构那侧错一位，配出来的链是假的（所以配对测试里刻意放了一个洞）。
- 顺带更正一处**写错的口径**：非字面量表名（`.from(TABLE)`）一直在射程内，只是标成 `<非字面量>`；
  原先的规则注释、报告页脚、testing.md、roadmap 都把它列成盲区。`docs/testing.md` 与
  `docs/roadmap-0.12.0.md` 的盲区清单已按实测重写。
- 留给 #49 的新证据：`dashboard/page.tsx:75` 那条 `as unknown as { data: … }` 是断言抹掉 `error`
  （C08 那一族的正主），但 C08 门禁要求断言外面套 `await`，这里 `await` 落在 `Promise.all` 上——
  **它只对 C08-c 可见，对门禁不可见**。接线时这一类必须一起判，否则门禁比测量工具还窄。
- 验证：`pnpm -s lint` / `pnpm -s type-check` → exit 0；`CI=true pnpm check:all` → **exit 0**
  （38 道门禁 / 210 个测试文件全过，规则单测 26 条）；`pnpm build` → exit 0。
  变异核对 8 项（W1–W8）全部被杀死且红在该判据上。
  **其中 W5 第一版逃过了变异**：我拿既有的「未 await 的构造器断言不算」当反向证据，
  而它绑的是标识符（`const query = …`），压根不走对象解构那条路 → 判据去掉 `await` 要求后仍全绿。
  补了一条 `const { data } = supabase.from(…)`（不 await）的用例之后才红。
  教训：**变异必须由走同一条代码路径的用例来接**，「有一条名字听起来对得上的用例」不算。
  顺手回去核了 CHANGELOG 里那条历史声明（V1–V4「去掉 `await` 要求」逐项红）——**它是真的**：
  那一版 mutate 的是「整个 `unwrapAwait` 不要了」，那种强法会把正向用例（总量 1 → 0）直接打死；
  W5 用的是更弱的写法（保留 await 命中、额外接受未 await 的），只有新加的那条才接得住。
  同一句「去掉 await 要求」在两种强度下是不同的变异，记这一点是为了下次别把两者当同一项。
- 下一步：按 24 处这份真实清单排批次——先 `actions/team.ts`(6) 与 `admin/page.tsx`(3)，
...
## 2026-09-23 — C08-c 第四批：管理概览页的三个数字

- 里程碑 / 版本：v0.12.0 / C08-c 清偿（#42 的后半）。
- 分支 / commit：`fix/c08c-admin-page`（栈在 #107 之上）。
- 状态：DONE（PR 待 review 合并）。
- 缺陷形状：`admin/page.tsx` 三处统计读数（`profiles` 计数、`teams` 计数、`profiles.role` 分布）
  都不绑 `error`，计数直接 `?? 0`。于是一次故障长得像**一个刚初始化的空实例**：
  0 个用户、0 个团队、0 个管理员。0 在这里是终态，没有人会去刷新重试，反而会去怀疑数据被清了。
- 改法：三处各自绑定 `error`，在渲染前 `throw`（`dashboard/error.tsx` 给重试入口）；
  「查询跑完了、计数就是 0」仍然是合法答案（第 2 条用例钉住）。
  守卫那条路原样保留：没权限走 `redirect`，而且**一次库都不读**（第 6 条用例钉住——
  证明我没有把一切都变成抛错）。
- 这个文件原先零单测。理由很老套：它是有权限才能看的页面，所以一直没人给它写桩。
  桩按「表名 + 该表第几次读取」预置（`profiles` 在一页里被读两次，必须能指名让其中一次失败），
  链必须可 `await`——页面直接 await `.select(...)`，缺 `then` 时故障用例会变成「什么都没读到」而假绿
  （同一个坑第三次出现：#101 的团队页、#105 的项目列表页、这里）。
- 「数字真的来自那次读取」这一条是靠**把元素树摊平成字符串**断言的（`textsOf`）。
  只断言 `resolves.toBeTruthy()` 的话，两种篡改都蒙得过去：角色行拿到了却不计数（A4）、
  读数写死成 0（A5）。这两项变异正是被那条断言杀死的，不是被那三条故障用例杀死的。
- 验证：`pnpm -s lint` / `pnpm -s type-check` → exit 0；`CI=true pnpm check:all` → **exit 0**（38 道门禁）；
  `pnpm build` → exit 0。新增 6 条用例；变异核对 A1–A6 **全部被杀死且红在该判据上**。
  （第一版脚本把 A4 的预期写成了一行注释里的文字，于是误报「红了但不是这条判据」——
  预期的必须是**用例名**，脚本自己也要走一遍这条路。）
- 数字同步：`--unbound` 18 → **15 处**。剩下：Stripe（checkout 2 + webhook 2）→ `dashboard/page.tsx`(5)
...
## 2026-09-23 — C08-c 第八批：E2E 种子端点，debt 到此清零

- 里程碑 / 版本：v0.12.0 / C08-c 清偿（#42 的后半，最后一批债务）。
- 分支 / commit：`fix/c08c-e2e-queue-reads`（栈在 #111 之上）。
- 状态：DONE（PR 待 review 合并）。
- 这批最严重的那处不是「显示错」，是**写坏数据**：`setPushPreference` 读的是它马上要覆盖的那一列。
  `notification_settings` 是装着多种偏好的 JSON 列，读失败时 `current` 落成 `{}`，
  紧接着的 update 就把用户其余偏好全部抹掉、只留下 push 一项。
  种子端点没有资格悄悄改写它没读到的东西——现在读不到就抛。
- 第二处：同一个文件的 GET 里，投递记录那条查询绑了 `error`、紧挨着的订阅那条没有。
  于是「订阅读不到」与「用户确实没有订阅」在 spec 里完全同形——**测试拿一份假的观察去断言真实行为**，
  这种缺陷比页面显示错更贵，因为它会让一条本来该红的 E2E 安静地绿。
- 「没覆盖」这件事状态码证明不了，所以那条用例断言的是**没有发出 `profiles` 的 update**
  （桩记下每次 `update(table)`）。这是本族测试里第一次需要断言「没写」而不是「返回什么」。
- 覆盖：6 条用例（这个端点此前零单测）；变异核对 **F1–F5 全部被杀死**：
  删两处新守卫、把两条合法分支（没有偏好行 / 没有订阅）说成故障、删掉原先那条已存在的守卫。
- **数字同步：`--unbound` 4 → 2 处，剩下的两处全是 `permission-gate.tsx` 的 `justified`**
  （客户端组件读角色失败时故意回落到最低权限，与 C08 台账同源）。
  也就是说 roadmap 里「清到只剩 `justified` 才接门禁」的那个条件**现在成立了**。
- 验证：`pnpm -s lint` / `pnpm -s type-check` → exit 0；`CI=true pnpm check:all` → **exit 0**（38 道门禁）；
  `pnpm build` → exit 0。
- 下一件事（顺序已在 roadmap 写定）：把 C08-c 判据接进 `check:query-errors`，
  同时给 `permission-gate.tsx` 那 2 处加**与 C08 同样的 `justified` 台账**（双向对账，防它烂成永久豁免表）。
- 更新时间：2026-09-23（UTC）。
...
- 台账从「一个文件一个数字」变成「一个文件、一条规则一个数字」：`ErrorChannelExemption` 加
  ...
  那两处语句**同时**是抹掉类型的断言和没绑 `error` 的解构，一条语句犯两条规则，
  所以两条台账各记一次。这不是把同一个数字抄两遍：接线前它俩本来就独立漂移
  （`--unbound` 数到 2，`check:query-errors` 也只数到 2，但那是两条判据各自的 2）。
- 新代码 `QUERY_ERROR_CHANNEL_UNBOUND_AWAY` / `_UNBOUND_STALE`，加一个解构侧的地板值
  `QUERY_ERROR_CHANNEL_UNBOUND_VACUOUS`。这个地板值是这次唯一真正需要新想的失败模式：
  **把 `readsFromPattern()` 调空，既有的 `judged > 0` 不会响**，因为断言那一半还在正常判到链——
  一条规则坏了，另一条的自检替它撒了谎。探针量过的三种坏法都响：删掉地板值（E）、
  门禁不再调用解构采集（M）、判据恒返回 `[]`（J，同时打红真实仓库用例与三条 fixture）。
  注意 J 不需要地板值也能被抓到（清单空了以后台账自然对不上），地板值真正独占的那一格是
  「解构一侧全库判不到任何读取」——此时台账为 0 的文件同样什么都报不出来。
- 地板值触发时的取舍写进了代码：仍然打印断言侧的违规（半个门禁坏了不能连带藏起另一半的结论），
  但**不**跑台账对账。后者是这次真正想清楚的一件事——采集器死掉时 `found` 处处为 0，
  每一条正当豁免都会显示成「过期条目，请删掉」，而照这句话去修就会把 `justified` 删没，
  下一批同类缺陷就此变成硬红。**门禁的提示语也是一段会被执行的代码**。
- 覆盖：单测 26 → 30 条。新增四条各自钉住一件事：一条语句两条规则各报一次（不是二选一）、
  两条台账相互独立（断言侧刚好、解构侧多一处，仍然要红——共用一个计数的实现在这里全绿）、
  解构债务还清不删台账要红、解构判据自己被调空要红且不许吞掉断言侧的结论。
  真实仓库那条用例同时改成**逐文件对两条台账**，并加了一条「台账之外不得有任何未绑 `error` 的解构」。
- 变异核对 13 项，**12 项被杀死**。存活的一项（D：把解构台账改成「只许多不许少」）没有放过去，
  而是另开一次探针查了它对应的断言侧写法——**同样存活**。也就是说这不是这次接线引入的洞，
  而是这一对规则本来就有的重叠：`found < allowed` 时 `*AWAY` 与 `*STALE` 都会报，
  去掉其中一个不会让门禁变绿，少掉的只是报告里的 `file:line`。这件事现在写在函数注释里，
  而不是留在「12/13」这个数字背后。
- 顺带改掉一条会过期的 roadmap 口径：`dashboard/page.tsx` 里那条 `Promise.all` 元素上的
  `as unknown as { data }`，从前写的是「只对 C08-c 可见、对门禁不可见」；接线之后它**已经对门禁可见**
  （解构那一半判得到），残留的只是断言那一半的瞎点，仍然归 #49。
- 事故与恢复（记在这里，因为它改变了下面那组数字的含义）：跑完变异矩阵之后，我为了回答
  「HEAD 上这个文件本来是不是 prettier-dirty」，把 `git show HEAD:<path>` 的产物 `cp` 回了工作区——
  一条只读问题，写坏了工作区，六处编辑的判据逻辑一次没掉（这是同一 reflex 的第六次，已写进长期记忆）。
  当时还有一个后台 `check:all` 正跑在被改坏的文件上，先看出的问题是它随后那次「绿」不能再引用。
  恢复方式是从对话里逐条重放那六次编辑，然后**重新证明**：门禁输出与 13 项变异矩阵（12 杀 / D 存活）
  与丢失前**逐项一致**，30 条单测同样一致——一致性比较才是「重放无损」的证据，不是我记得写过什么。
  顺带把那个 prettier 问题问完了：HEAD 上这个文件本来就不过 `--check`，仓库没有格式门禁，
  所以这次不顺手 `--write`（那会把一次逻辑改动混成一整篇格式化 diff）。
- 验证（重放之后重跑，全部在同一份最终代码上）：`pnpm -s lint` / `pnpm -s type-check` → exit 0；
  `node scripts/check-query-error-channel.js` →
  `358 个文件 / 3 处 awaited 断言 + 165 处 awaited 解构，台账 断言 2 / 解构 2`，exit 0（0.79s）；
