...
function parseArgs(argv) {
  ...
  const options = { url: undefined, expectedVersion: undefined, output: undefined, timeoutMs: DEFAULT_TIMEOUT_MS };
  let index = 0;
  ...
  if (options.expectedVersion === "") throw new Error("--expected-version requires a non-empty value");
  if (options.output === "") throw new Error("--output requires a non-empty value");

  options.url ??= process.env.PRODUCTION_URL;
  if (options.expectedVersion === undefined) options.expectedVersion = process.env.EXPECTED_APP_VERSION;
  return options;
}
...
function parseBaseUrl(value) {
  ...
  if (parsed.username || parsed.password) throw new Error("Production URL must not contain credentials");
  parsed.search = "";
  parsed.hash = "";
...
function healthFailureDetail(response, body, versionMatches, expectedVersion) {
  ...
  const failed = checks.filter(([, passed]) => !passed).map(([label, passed]) => (passed ? null : label));
  return failed.filter((label) => label !== null).join(", ") || "invalid health response";
}

async function checkHealth(baseUrl, options) {
  const response = await request(options.fetchImpl, joinUrl(baseUrl, "/api/health"), {
    method: "GET",
    redirect: "manual",
  ...
  const passed =
    ...
    versionMatches;
  const detail = passed
    ? `HTTP 200, status=ok, ready=true, version=${body.version}`
    : healthFailureDetail(response, body, versionMatches, options.expectedVersion);
  return result("health", passed, detail, response.status, { version: body?.version ?? null });
}

async function checkHomepage(baseUrl, options) {
  const response = await request(options.fetchImpl, joinUrl(baseUrl, "/"), {
    method: "GET",
    redirect: "manual",
...
async function checkStaticAsset(baseUrl, options) {
  const response = await request(options.fetchImpl, joinUrl(baseUrl, "/icon.svg"), {
    method: "GET",
    redirect: "manual",
  }, options.timeoutMs);
  ...
  return result(
    "static-asset",
    passed,
    passed ? "HTTP 200, icon.svg served as SVG" : `HTTP ${response.status}, content-type=${response.headers.get("content-type") ?? "missing"}`,
    response.status,
  );
}
...
async function checkSecurityHeaders(baseUrl, options) {
  ...
  const checks = {
    csp: headerIncludes(headers, "content-security-policy", "default-src 'self'") && headerIncludes(headers, "content-security-policy", "frame-ancestors 'none'"),
    hsts: /max-age=(\d+)/.test(headers.get("strict-transport-security") ?? "") && Number(RegExp.$1) >= 31_536_000,
  ...
  return result(
    ...
    failed.length ? `missing/invalid: ${failed.join(", ")}` : "CSP, HSTS, nosniff, frame, referrer, permissions and request-id present",
    response.status,
    { checks },
  );
}
...
async function checkUnauthorizedDashboard(baseUrl, options) {
  const response = await request(options.fetchImpl, joinUrl(baseUrl, "/dashboard"), {
    method: "GET",
    redirect: "manual",
    headers: { Accept: "text/html" },
  }, options.timeoutMs);
  ...
  return result(
    "anonymous-dashboard",
    passed,
    passed ? `HTTP ${response.status} redirects to /auth/login` : `HTTP ${response.status}, location=${location || "missing"}`,
    response.status,
...
async function checkWebhookRejection(baseUrl, options) {
  const response = await request(options.fetchImpl, joinUrl(baseUrl, "/api/webhooks/stripe"), {
    method: "POST",
    redirect: "manual",
  ...
  const body = await response.json().catch(() => null);
  const passed = response.status === 400 && body?.error === "Missing signature" && headerIncludes(response.headers, "cache-control", "no-store");
  return result(
    "webhook-signature-rejection",
    passed,
    passed ? "HTTP 400, missing signature rejected without side effects" : `HTTP ${response.status}, error=${body?.error ?? "invalid body"}`,
    response.status,
  );
}
...
async function runProductionSmoke(baseUrlValue, options = {}) {
  ...
  if (!Number.isFinite(config.timeoutMs) || config.timeoutMs <= 0) throw new Error("timeoutMs must be a positive number");
  ...
  for (const runner of runners) {
    try {
      ...
      checks.push(result(runner.name, false, error instanceof Error ? error.message : String(error), null));
    }
  }
...
async function main() {
  ...
  if (!options.url) {
    console.error("Usage: pnpm smoke:production -- https://example.com [--expected-version 0.6.0] [--output smoke.json]");
    return 2;
  }
  ...
  console.log(`${report.passed ? "✅" : "❌"} production smoke: ${passed}/${report.checks.length} passed (${report.baseUrl})`);
