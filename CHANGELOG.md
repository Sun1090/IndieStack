...
- **C08 的邻居缺陷现在可以量了（先量，不先接门禁）**：`query-error-channel` 多了一条纯规则
  ...
  重量结果以 `--unbound` 的输出为准（roadmap C08-c 记录了口径；**这一版少判了三类写法**，
  读数是下界，更正见下面的 Fixed），抽样逐行看过 5 处，**误报 0**——全是同一个方向：
  读失败长成「没有这个团队 / 0 个用户 / 这个人不是管理员 / 这张发票没有归属」。
  报告页脚同时写明「哪些没测」：那一版列的三条里，「非字面量表名的链」其实是写错的
  （`.from(TABLE)` 一直在射程内），另两条（`Promise.all` 里的查询链、条件表达式包住的链）
  是真的漏判，已在下面的 Fixed 里补上；「绑了 `error` 却从不使用」那一档**刻意不测**——
  判它需要作用域分析，全文数同名标识符会把 `catch (error)` 一起数进去，
  那是个只会漏报的假指标，而一个只会漏报的计数比没有计数更糟。
...
- **结账的两处读取原先的故障方向是「放行」**（C08-c 第五批）：
  ...
  这类缺陷比「把有说成无」更贵：它不是显示错误，是钱和数据状态真的会分叉。
  现在两处都在建会话之前回 **503 + `checkoutUnavailable`**（新增错误码，双语），
  并且新增的用例同时断言 `createCheckoutSession` **一次都没被调用**。
  这个路由**原先零单测**。补齐 5 条：两处故障、两处合法终态（个人用户没有团队照旧放行、
  已有有效订阅照旧 409）、以及守卫未登录仍 401。
  两处守卫加进来之后 `POST` 的 `complexity` 到 16（上限 15），于是把「谁在买、已经在买了吗」
  抽成 `readCheckoutScope()` 返回三态——**这条门禁每次都在指出「这个函数已经在做第二件事」**，
  不是形式问题。名字与栈里的 #96 撞了（那条 PR 在同一路由上抽的是同一个概念），
  合并时以先落地的那份为准即可，不是两份不同的东西。
- **付款成功的 owner 通知不再安静地消失**（同批）：`api/webhooks/stripe` 的
  `notifyTeamOwner` 两次读取（按 `provider_id` 回查订阅归属、查团队 owner）都不绑 `error`，
  读失败与「这个订阅不属于任何团队」「这个团队没有 owner」长得一模一样——钱收到了，没人被通知，
  日志里什么都没有。现在两处各自点名上报后返回（通知失败不让 Stripe 重放整个事件，那是 200 的语义）。
  变异核对 7 项（C1–C4、W1–W3）全部被杀死：删两处守卫、把可重试的故障说成 403、
  scope 检查整个跳过、两处安静返回、以及抛给外层 catch 让日志标签退化成通用那句。
  规模同步：`--unbound` 15 → **11 处**。
...
- **C08-c 的读数原先是下界，现在补齐了三类写法**：`collectUnboundErrorChannels` 只认
  ...
  一条都判不到——上一批清项目页时撞出来的（那 4 处只让清单降了 3）。补齐后重量
  358 文件 / **165 处**解构 / **24 处不绑 `error`**（原先报 160 / 20），新量出的 4 处逐行看过、误报 0。
  两个实现细节值得记：`Promise.all` 的参数是**一个数组字面量**而不是可变参数，第一版按
  `node.arguments` 配对，结果一条链都不认识——是探针脚本抓出来的，不是测试；数组字面量里的空缺位
  不能滤掉，滤掉之后下标与解构那侧错一位，配出来的链就是假的。
  顺带改掉一处写错的口径：非字面量表名（`.from(TABLE)`）**一直**在射程内，只是标成 `<非字面量>`，
  文档与报告页脚原先把它列成盲区。`unwrapExpression` 因此把 `await ( … )` 也穿了过去
  （C08 门禁的判定数字没变，实测仍是一致）。
  变异核对 8 项（W1–W8）全部被抓：忘了数组字面量展开、不筛嵌套数组解构、只走条件的 then 支、
  漏传外层 `await`、去掉 `await` 要求、把 `allSettled` 当 `all`、配对不按解构下标、整支失效。
  其中「去掉 `await` 要求」这一项**第一版逃过了变异**：我拿「未 await 的构造器断言不算」那条用例当
  反向证据，而它绑的是标识符（`const query = …`），压根不走对象解构那条路——补了一条
  `const { data } = supabase.from(…)`（不 await）的用例之后它才红。判据的变异必须由
  **走同一条代码路径**的用例来接：同一句「去掉 `await` 要求」，整个不要 `unwrapAwait` 是强变异
  （正向用例当场打死），只放开「未 await 也算」是弱变异（只有新加的用例接得住）——两者不是一回事。
...
- **API 密钥的两个动作不再报告它们没做到的事**：
  ...
  现在它 `.select("id")` 把改掉的行拿回来并返回布尔，调用方据此回答 `apiKeyNotFound`。
  `regenerateApiKey` 的问题在**顺序**：原先先签发新密钥、再吊销旧密钥，而两步都不接结果——
  如果第二步抛错，就留下一个**用户从没见过明文、却处于 active 的密钥**（明文只在成功响应里给一次），
  同时旧密钥仍然有效，而动作以一个未处理异常收场。现在改成**先吊销、再签发**：
  最坏情况变成「旧的回不去、新的没出来」，那是一次**看得见**的失败，并且用户手里就有
  「创建密钥」这条出路。为此新增专用错误码 `apiKeyRevokedButNotCreated`
  （「旧密钥已吊销，但新密钥签发失败，请重新创建一个密钥。」）——不能用泛化的 `databaseError`，
  因为它说的不是「什么都没变」。台账之外的一类：**写操作的成功与否要由结果行数决定，不是由有没有 `error` 决定**。
  覆盖：仓库层 3 条（含「0 行受影响返回 false」与一条钉住 `.select()` 真的存在的断言——
  共享的 `chainMock` 所有 builder 都返回同一个链，不显式钉就看不见少掉的那一环），
  动作层 3 条（0 行不改写页面、签发失败的专用回答、成功路径的调用顺序 `["update","insert"]`）。
  变异核对 6 项（K1–K6）：返回值恒真、删掉 `.select()`、不数行数、退回泛化错误码、
  顺序退回「先签发再吊销」各红对应用例，最后一条是反向证据（把真的吊销成功也判成找不到会红）。

- **最后一批「读失败说成一个结论」：团队页、账单页与用量图**（C08-b 第六批，**debt 清零**）：
  `dashboard/team/page.tsx` 三处、`dashboard/billing/page.tsx` 一处、`api/analytics/route.ts` 两处。
  页面上的读失败原先一律长成**合法的读数**：团队页把一次抖动渲染成「你还没有团队」加一个「创建团队」
  按钮（用户于是相信自己没有团队，最坏情况是再建一个）；成员列表那处（**没有类型断言，C08 门禁看不见**）
  把满员的团队显示成 0 人；账单页 `currentPlan = teamInfo?.plan ?? "free"` 让**付费账户显示成免费**；
  用量接口把两次读取的失败拼成「0 个请求、0 个错误、空时间线」——对一个正在用密钥的账户，
  这就是「你的密钥没人用」。现在三处页面读取在渲染前抛出，交给 `dashboard/error.tsx`
  （可重试，走 `errors.errorBoundary.*` 文案）；接口回 **503** + 一句「…Please retry.」。
  「没有这一行」照旧是合法状态：没有成员关系仍渲染空态、团队行缺失仍按现有回退显示、
  真的零调用仍是零值图。
  用量接口那处保留一个**带 `error` 成员**的结果类型断言——这条链 await 下来是 `any`
  （列名串没匹配上生成的关系类型），而 `any` 正是「读失败看不见」的成因；
  门禁要的是错误通道还在，不是某种特定写法。
  覆盖：`team/page.test.tsx`（新建 6 条）、`billing/page.test.tsx`（新建 3 条）、
  `analytics/route.test.ts`（+3 条），故障与合法状态成对。变异核对 8 项（P1–P8）：
  六处 guard 空转各红自己那条，两条反向证据（把「确实没有团队」抛掉、把「真的零调用」判成 503）会红；
  后者一次红 3 条，因为它把所有零用量账户一起挡在门外——一次变异未必只对应一条用例，
  这里要的是「它一定会红」。台账 6 → 2 处，剩下的两条是 `permission-gate.tsx` 的 `justified`
  （客户端组件无法 5xx，读角色失败回落最低权限是刻意的），**C08-b 的债务到此清完**。
  测试桩交出的两条教训：假客户端的查询链**必须可 await**（少一个 `then` 时，列表那一路 await 到
  一个普通对象，`data`/`error` 双双 `undefined`，6 条用例里只有 1 条发现它）；
  局部 `pnpm type-check` 也救过一次——`dailyData` 变 `any` 后 `.map((row) => …)` 隐式 any 只有 tsc 报得出来。

- **会话吊销与密钥重新生成不再把读失败说成一个确定的结论**（C08-b 第五批，②的收尾）：
  `revokeSession` 里那次「这台设备的会话记录在不在」的读取原先被断言成不含 `error` 的类型，读失败回
  `sessionNotFound`，而该文案说的是「它可能已经被吊销了」——**这不只是一次措辞问题**：一次数据库抖动会让
  用户以为某台设备已经登出，而那个会话可能还好端端地活着，他既不会重试也不会去查第二遍。
  现在读失败记日志并回 `databaseError`（可重试），删除动作一次都不发生。
  `regenerateApiKey` 原先把两件事共用一个 `databaseError`：「这个密钥不存在」与「没读到它的元数据」。
  后者现在各自回答（`databaseError` 可重试 / 新增的 `apiKeyNotFound` 是终态，en/zh-CN 各一条文案），
  且**读失败时一个密钥都不签发、也不吊销旧的**——名字与 scopes 正是从这一行读来的，
  没读到就签发会产出一个权限错的密钥而不是失败。台账 8 → 6 处（debt 6 → 4，剩两处是 `justified`）。
  补 4 条用例（会话读失败 1 条、密钥读失败 1 条、密钥确实不存在 1 条、正向签发 1 条）。
  正向那条断言的是**写进去的载荷**而不是返回值：mock 的 insert 固定回一条 `name: "New"`，
  拿返回值断言名字等于什么都没断——这一条是它自己第一次跑红才暴露的。
  合法状态一侧沿用既有那条「会话不存在返回 `sessionNotFound`」，与新增的故障用例配成对。
  变异核对 4 项（N1–N4）：两处 guard 空转各红自己那条，两处「把合法状态也改成故障」是反向证据。
