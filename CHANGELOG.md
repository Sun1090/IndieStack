...
- **每条 API 路由靠什么保护，现在是一份会被核对的台账**（C11）：新增 `pnpm check:route-auth`。
  起因不是假设，是两件事叠在一起：`src/proxy.ts` 的 `protectedRoutes` 只有 `/dashboard` 与
  `/dashboard/(.*)`，**`/api/*` 一条都不在里面**（页面级重定向很容易让人以为 API 也被挡住了）；而
  `api/e2e/email-inbox` 的 GET 在 2026-09-24 之前确实没有任何鉴权、同文件的 POST/DELETE 却有——收件箱里
  ...
  三处失明是边写边撞出来的：① `symbol in PROTECTION_SYMBOLS` 顺原型链命中，于是每个文件都「有守卫」
  （`x.toString()` 就够），改成自有键判定；② 营销退订的仓库函数 `unsubscribeByToken` 一开始写成
