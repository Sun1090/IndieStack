...
- **进度台账第一次有自己的门禁**（C08 主题的自反应用）：新增 `pnpm check:progress`，
  ...
  搬运时一个脚本用切片重组整个文件，边界取错把 main 的最后一条整条删掉（该文件相对 HEAD
  仍是干净的，`git checkout --` 才救回来）。两条判定各对应一次实际事故——不是假想敌。
  门禁先在坏样本上跑红：把今天的条目插到顶部报 `date-out-of-order`（指出倒挂发生在第几行与
  它上一条的日期），把同一条目写两遍报 `duplicate-heading`（指回第一次出现的行号）；
  在当前 main 与这条分支的真实台账上都必须绿。四条判定各做变异核对：关掉重复检测 → 1 项红、
  关掉日期检测 → 3 项红、必填字段清空 → 2 项红、空台账不再失败封闭 → 1 项红；把字段匹配从
  「行首标签前缀」退回「整名相等」（真实台账写的是 `- 里程碑 / 版本：`）→ 9 项红，
  包括那条「读真文件」的用例。每项跑完从 `/tmp` 字节副本还原并用 `cmp` 确认一致。
